...
def crop_rir(rir, target_length):
    ...
    # First align the start of the rir 
    rir = rir[0]
    onset = find_direct_location(rir)
...
def find_direct_location(rir):
    ...
    direct_location_candidate_list = np.argwhere(np.array(rir[:peak_val_location])> threshold_val)
    if len(direct_location_candidate_list) == 0 : 
        return peak_val_location
    else : 
        return direct_location_candidate_list[0][0]


def get_direct_and_later_part(rir, fs):
    direct_length = int(fs * 0.0005)
    direct_location = find_direct_location(rir)
    hanning_window = np.hanning(direct_length*2)
    fadeout_window = hanning_window[direct_length:]
    fadein_window = hanning_window[:direct_length]
    direct_window = np.ones(direct_location + direct_length) 
    print(len(rir), len(direct_window))
    later_window = np.ones(len(rir) - len(direct_window))

    direct_window[direct_location:direct_location + direct_length] = fadeout_window
    later_window[:direct_length] = fadein_window 
    direct = np.zeros_like(rir)
   
    direct[:direct_location + direct_length] = rir[:direct_location + direct_length] * direct_window
    later = rir[direct_location+direct_length:] * later_window
    return direct, later
...
def audio_normalize_batch(sig, type, rms_level=0.1, peak_val=1.0):
    if type == "peak":
        ...
        return noramlized_energy_batch(sig)
    else:
